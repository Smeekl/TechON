...
Class Products {
    /**
     * __construct - конструктор класса, принимает массив с нашей "бд"
     * и записывает его в приватную переменную $array.
     *
     * getSortArrayByValue($key, $sortType) - сортирует нашу базу по указанному ключу,
     * в зависимости от второго необязательного параметра. Для сортировки по убыванию, необходимо задать
     * параметр 'highest' для $sortType.
     *
     * getElementByID($id) - принимает параметр ID и возвращает подходящий массив.
     *
     **/
    ...
    public function getSortArrayByKey($key,$sortType = null){
        if ($sortType == 'highest') {
        ...
        } else {function array_sorter($key) {
            return function ($a, $b) use ($key) {
                return strnatcmp($a[$key], $b[$key]);
            };
        }}

        usort($this->array, array_sorter($key));
